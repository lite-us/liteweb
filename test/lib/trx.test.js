...
describe('LiteWeb.xlt', function () {
    ...
    describe('#constructor()', function () {
        ...
        it('should have been set a full instance in liteWeb', function () {
            assert.instanceOf(liteWeb.xlt, LiteWeb.Xlt);
        });
    ...
    describe('#Account Test', function () {
        ...
        describe("#getAccount", async function () {
            ...
            it('should get account by hex or base58 address', async function () {
                ...
                for (let type of addressType) {
                    account = await liteWeb.xlt.getAccount(accounts[type][idx]);
            ...
            it('should throw address is not valid error', async function () {
                await assertThrow(
                    liteWeb.xlt.getAccount('notAnAddress'),
        ...
        describe("#getAccountById", async function () {
            ...
            before(async function(){
                this.timeout(10000);
                accountId = LiteWeb.toHex(`testtest${Math.ceil(Math.random()*100)}`);
                const transaction = await liteWeb.transactionBuilder.setAccountId(accountId, accounts.hex[idx]);
            ...
            it('should get confirmed account by id', async function () {
                this.timeout(20000);
                while (true) {
                    const account = await liteWeb.xlt.getAccountById(accountId);
        ...
        describe("#getAccountResources", async function () {
            ...
            it('should get account resource by hex or base58 address', async function () {
                ...
                let accountResource;
                for (let type of addressType) {
                    accountResource = await liteWeb.xlt.getAccountResources(accounts[type][idx]);
            ...
            it('should throw address is not valid error', async function () {
                await assertThrow(
                    liteWeb.xlt.getAccountResources('notAnAddress'),
                    'Invalid address provided'
                );
            });
        ...
        describe("#getBalance", async function () {
            ...
            it('should get balance by hex or base58 address', async function () {
                ...
                let balance;
                for (let type of addressType) {
                    balance = await liteWeb.xlt.getBalance(accounts[type][idx]);
        ...
        describe("#getBandwidth", async function () {
            ...
            it('should get bandwith by hex or base58 address', async function () {
                ...
                let bp;
                for (let type of addressType) {
                    bp = await liteWeb.xlt.getBandwidth(accounts[type][idx]);
                    assert.isTrue(bp >= 0);
                }
            });
        ...
        describe("#getUnconfirmedAccount", async function () {
            ...
            before(async function(){
                ...
                const account = await liteWeb.createAccount();
                toHex = account.address.hex;
                const transaction = await liteWeb.transactionBuilder.sendXlt(account.address.hex, 10e5, accounts.hex[idx]);
                await broadcaster(null, accounts.pks[idx], transaction);
                await waitChainData('account', account.address.hex);
            });

            it('should get unconfirmed account by address', async function () {
                const account = await liteWeb.xlt.getUnconfirmedAccount(toHex);
                assert.equal(account.address, toHex.toLowerCase());
            });

            it('should throw address is not valid error', async function () {
                await assertThrow(
                    liteWeb.xlt.getUnconfirmedAccount('notAnAddress'),
                    'Invalid address provided'
                );
            });
        ...
        describe("#geUnconfirmedAccountById", async function () {
            ...
            it('should get unconfirmed account by id', async function () {
                ...
                const account = await liteWeb.xlt.getUnconfirmedAccountById(accountId);
                assert.equal(account.account_id, accountId.slice(2));
            });
            ...
            it('should throw accountId is not valid error', async function () {
                ...
                for (let id of ids) {
                    await assertThrow(
                        liteWeb.xlt.getUnconfirmedAccountById(id),
                        'Invalid accountId provided'
                    );
                }
            });
        ...
        describe("#getUnconfirmedBalance", async function () {
            ...
            it('should get unconfirmed balance by account address', async function () {
                const balance = await liteWeb.xlt.getUnconfirmedBalance(toHex);
        ...
        describe("#updateAccount", async function () {
            ...
            it('should update account name', async function () {
                ...
                await liteWeb.xlt.updateAccount(accountName, { privateKey: accounts.pks[idx], address: accounts.hex[idx] });
                const account = await liteWeb.xlt.getUnconfirmedAccount(accounts.hex[idx]);
                assert.equal(liteWeb.toUtf8(account.account_name), accountName);
            });

            it('should throw name must be a string error', async function () {
                await assertThrow(
                    liteWeb.xlt.updateAccount({}),
    ...
    describe('#Sign Test', function () {
        ...
        describe("#sign", async function () {
            ...
            beforeEach(async function() {
                transaction = await liteWeb.transactionBuilder.freezeBalance(10e5, 3, 'BANDWIDTH', accounts.b58[idx]);
            });

            it('should sign a transaction', async function () {
                const signedTransaction = await liteWeb.xlt.sign(transaction, accounts.pks[idx]);
            ...
            it('should throw transaction not valid error', async function () {
                await assertThrow(
                    liteWeb.xlt.sign(undefined, accounts.pks[idx]),
            ...
            it('should throw transaction is already signed error', async function () {
                const signedTransaction = await liteWeb.xlt.sign(transaction, accounts.pks[idx]);
                await assertThrow(
                    liteWeb.xlt.sign(signedTransaction, accounts.pks[idx]),
            ...
            it('should throw private key does not match address error', async function () {
                await assertThrow(
                    liteWeb.xlt.sign(transaction, accounts.pks[idx]),
        ...
        describe("#signMessage", async function () {
            ...
            it('should sign a hex string message', async function () {
                ...
                const signedMsg = await liteWeb.xlt.sign(hexMsg, accounts.pks[idx]);
            ...
            it('should throw expected hex message input error', async function () {
                ...
                await assertThrow(
                    liteWeb.xlt.sign(hexMsg, accounts.pks[idx]),
                    'Private key does not match address in transaction'
                );
            });
        ...
        describe("#verifyMessage", async function () {
            ...
            before(async function() {
                ...
                signedMsg = await liteWeb.xlt.sign(hexMsg, accounts.pks[idx], null, false);
            });

            it('should verify signature of signed string message', async function () {
                const result = await liteWeb.xlt.verifyMessage(hexMsg, signedMsg, accounts.hex[idx], null);
                assert.isTrue(result);
            });

            it('should throw expected hex message input error', async function () {
                await assertThrow(
                    liteWeb.xlt.verifyMessage('e66f4c8f323229131006ad3e4a2ca65dfdf339f0', signedMsg, accounts.hex[idx], null),
            ...
            it('should throw signature does not match error', async function () {
                ...
                await assertThrow(
                    liteWeb.xlt.verifyMessage(hexMsg, fakeSig, accounts.hex[idx], null),
        ...
        describe("#multiSignTransaction", async function () {
            ...
            before(async function() {
                ...
                const updateTransaction = await liteWeb.transactionBuilder.updateAccountPermissions(
                    ownerAddress,
                    ownerPermission,
                    null,
                    [activePermission]
                ...
                const signedUpdateTransaction = await liteWeb.xlt.sign(updateTransaction, ownerPk, null, false);
                await liteWeb.xlt.broadcast(signedUpdateTransaction);
            ...
            it('should multi-sign a transaction by owner permission', async function () {
                ...
                const transaction = await liteWeb.transactionBuilder.freezeBalance(10e5, 3, 'BANDWIDTH', accounts.b58[ownerIdx]);
                let signedTransaction = transaction;
                for (let i = idxS; i < idxE; i++) {
                    signedTransaction = await liteWeb.xlt.multiSign(signedTransaction, accounts.pks[i], 0);
                }

                assert.equal(signedTransaction.signature.length, 3);

                // broadcast multi-sign transaction
                const result = await liteWeb.xlt.broadcast(signedTransaction);
            ...
            it('should multi-sign a transaction by owner permission (permission id inside tx)', async function () {
                ...
                const transaction = await liteWeb.transactionBuilder.freezeBalance(10e5, 3, 'BANDWIDTH', accounts.b58[ownerIdx], {permissionId: 0});
                let signedTransaction = transaction;
                for (let i = idxS; i < idxE; i++) {
                    signedTransaction = await liteWeb.xlt.multiSign(signedTransaction, accounts.pks[i]);
                }
            ...
            it('should verify weight after multi-sign by owner permission', async function () {
                ...
                for (let i = idxS; i < idxE; i++) {
                    signedTransaction = await liteWeb.xlt.multiSign(signedTransaction, accounts.pks[i], 0);
                    signWeight = await liteWeb.xlt.getSignWeight(signedTransaction);
                    if (i < idxE - 1) {
                ...
                // get approved list
                const approvedList = await liteWeb.xlt.getApprovedList(signedTransaction);
            ...
            it('should multi-sign a transaction with no permission error by owner permission', async function () {
                ...
                const transaction = await liteWeb.transactionBuilder.freezeBalance(10e5, 3, 'BANDWIDTH', accounts.b58[ownerIdx]);
                try {
                    await liteWeb.xlt.multiSign(transaction, (accounts.pks[ownerIdx] + '123'), 0);
                } catch (e) {
            ...
            it('should multi-sign duplicated a transaction by owner permission', async function () {
                ...
                const transaction = await liteWeb.transactionBuilder.freezeBalance(10e5, 3, 'BANDWIDTH', accounts.b58[ownerIdx]);
                try {
                    let signedTransaction = await liteWeb.xlt.multiSign(transaction, accounts.pks[ownerIdx], 0);
                    await liteWeb.xlt.multiSign(signedTransaction, accounts.pks[ownerIdx], 0);
                } catch (e) {
            ...
            it('should multi-sign a transaction by active permission', async function () {
                ...
                let signedTransaction = transaction;
                for (let i = idxS; i < idxE; i++) {
                    signedTransaction = await liteWeb.xlt.multiSign(signedTransaction, accounts.pks[i], 2);
                }
            ...
            it('should multi-sign a transaction by active permission (permission id inside tx)', async function () {
                ...
                const transaction = await liteWeb.transactionBuilder.freezeBalance(10e5, 3, 'BANDWIDTH', accounts.b58[ownerIdx], {permissionId: 2});
                let signedTransaction = transaction;
                for (let i = idxS; i < idxE; i++) {
                    signedTransaction = await liteWeb.xlt.multiSign(signedTransaction, accounts.pks[i]);
                }
            ...
            it('should verify weight after multi-sign by active permission', async function () {
                ...
                for (let i = idxS; i < idxE; i++) {
                    signedTransaction = await liteWeb.xlt.multiSign(signedTransaction, accounts.pks[i], 2);
                    signWeight = await liteWeb.xlt.getSignWeight(signedTransaction, 2);
                    if (i < idxE - 1) {
                        assert.equal(signWeight.result.code, 'NOT_ENOUGH_PERMISSION');
                    }
                    assert.equal(signWeight.approved_list.length, i - idxS + 1);
            ...
            it('should multi-sign a transaction with no permission error by active permission', async function () {
                ...
                const transaction = await liteWeb.transactionBuilder.freezeBalance(10e5, 3, 'BANDWIDTH', accounts.b58[ownerIdx]);
                try {
                    await liteWeb.xlt.multiSign(transaction, (accounts.pks[ownerIdx] + '123'), 2);
                } catch (e) {
                    assert.isTrue(e.indexOf('has no permission to sign') != -1);
                }
            ...
            it('should multi-sign duplicated a transaction by active permission', async function () {
                ...
                const transaction = await liteWeb.transactionBuilder.freezeBalance(10e5, 3, 'BANDWIDTH', accounts.b58[ownerIdx]);
                try {
                    let signedTransaction = await liteWeb.xlt.multiSign(transaction, accounts.pks[ownerIdx], 2);
                    await liteWeb.xlt.multiSign(signedTransaction, accounts.pks[ownerIdx], 2);
                } catch (e) {
                    assert.isTrue(e.indexOf('already sign transaction') != -1);
                }
            ...
            it('should multi-sign a transaction with wrong permission id error', async function () {
                ...
                try {
                    const transaction = await liteWeb.transactionBuilder.freezeBalance(10e5, 3, 'BANDWIDTH', accounts.b58[ownerIdx]);
                    await liteWeb.xlt.multiSign(transaction, accounts.pks[ownerIdx], 1);
                } catch (e) {
    ...
    describe('#Block Test', function () {
        ...
        describe("#getBlock", async function () {
            ...
            it('should get earliest or latest block', async function () {
                ...
                for (let type of blockType) {
                    block = await liteWeb.xlt.getBlock(type);
                    if (type === 'earliest') {
        ...
        describe("#getBlockByHash", async function () {
            ...
            it('should get block by block hash (id)', async function () {
                const block = await liteWeb.xlt.getBlock('latest');
                const blockByHash = await liteWeb.xlt.getBlockByHash(block.blockID);
        ...
        describe("#getBlockByNumber", async function () {
            ...
            it('should get block by block number', async function () {
                const block = await liteWeb.xlt.getBlock('latest');
                const blockByNumber = await liteWeb.xlt.getBlockByNumber(block.block_header.raw_data.number);
        ...
        describe("#getBlockRange", async function () {
            ...
            it('should get block by range', async function () {
                const blocks = await liteWeb.xlt.getBlockRange(0, 5);
            ...
            it('should get invalid start or end error by range', async function () {
                ...
                for (let range of ranges) {
                    await assertThrow(
                        liteWeb.xlt.getBlockRange(range[0], range[1]),
        ...
        describe("#getBlockTransactionCount", async function () {
            ...
            it('should get transaction count by block number, \'latest\' or \'earliest\'', async function () {
                ...
                for (let type of blockType) {
                    const count = await liteWeb.xlt.getBlockTransactionCount(type);
        ...
        describe("#getCurrentBlock", async function () {
            ...
            it('should get current block', async function () {
                const block = await liteWeb.xlt.getCurrentBlock();
                assert.isNumber(block.block_header.raw_data.number);
            });
    ...
    describe('#Transaction Test', function () {
        ...
        describe("#send", async function () {
            ...
            it('should send xlt', async function () {
                this.timeout(10000);

                const balanceBefore = await liteWeb.xlt.getUnconfirmedBalance(accounts.hex[toIdx]);
                await liteWeb.xlt.send(accounts.hex[toIdx], 10e5, { privateKey: accounts.pks[fromIdx], address: accounts.hex[fromIdx] });
                await waitChainData('balance', accounts.hex[toIdx], balanceBefore);
                const balanceAfter = await liteWeb.xlt.getUnconfirmedBalance(accounts.hex[toIdx]);
                assert.equal(balanceAfter - balanceBefore, 10e5);
            });

            it('should throw invalid recipient provided error', async function () {
                await assertThrow(
                    liteWeb.xlt.send('notValidAddress', 10e5, { privateKey: accounts.pks[fromIdx] }),
            ...
            it('should throw invalid amount provided error', async function () {
                await assertThrow(
                    liteWeb.xlt.send(accounts.hex[toIdx], -1, { privateKey: accounts.pks[fromIdx] }),
        ...
        describe("#sendTransaction", async function () {
            ...
            it('should send xlt', async function () {
                ...
                const balanceBefore = await liteWeb.xlt.getUnconfirmedBalance(accounts.hex[toIdx]);
                await liteWeb.xlt.sendTransaction(accounts.hex[toIdx], 10e5, { privateKey: accounts.pks[fromIdx] });
                await waitChainData('balance', accounts.hex[toIdx], balanceBefore);
                const balanceAfter = await liteWeb.xlt.getUnconfirmedBalance(accounts.hex[toIdx]);
                assert.equal(balanceAfter - balanceBefore, 10e5);
            });
            ...
            it('should throw invalid recipient provided error', async function () {
                await assertThrow(
                    liteWeb.xlt.sendTransaction('notValidAddress', 10e5, { privateKey: accounts.pks[fromIdx] }),
                    'Invalid recipient provided'
                );
            });
            ...
            it('should throw invalid amount provided error', async function () {
                await assertThrow(
                    liteWeb.xlt.sendTransaction(accounts.hex[toIdx], -1, { privateKey: accounts.pks[fromIdx] }),
                    'Invalid amount provided'
                );
            });
        ...
        describe("#sendXlt", async function () {
            ...
            it('should send xlt', async function () {
                ...
                const balanceBefore = await liteWeb.xlt.getUnconfirmedBalance(accounts.hex[toIdx]);
                await liteWeb.xlt.sendXlt(accounts.hex[toIdx], 10e5, { privateKey: accounts.pks[fromIdx], address: accounts.hex[fromIdx] });
                await waitChainData('balance', accounts.hex[toIdx], balanceBefore);
                const balanceAfter = await liteWeb.xlt.getUnconfirmedBalance(accounts.hex[toIdx]);
                assert.equal(balanceAfter - balanceBefore, 10e5);
            });
            ...
            it('should throw invalid recipient provided error', async function () {
                await assertThrow(
                    liteWeb.xlt.sendXlt('notValidAddress', 10e5, { privateKey: accounts.pks[fromIdx] }),
                    'Invalid recipient provided'
                );
            });
            ...
            it('should throw invalid amount provided error', async function () {
                await assertThrow(
                    liteWeb.xlt.sendXlt(accounts.hex[18], -1, { privateKey: accounts.pks[fromIdx] }),
                    'Invalid amount provided'
                );
            });
        ...
        describe("#freezeBalance", async function () {
            ...
            it('should freeze balance for energy or bandwidth', async function () {
                ...
                let accountBefore = await liteWeb.xlt.getAccount(accounts.hex[idx]);
                await liteWeb.xlt.freezeBalance(10e5, 3, 'BANDWIDTH', { privateKey: accounts.pks[idx], address: accounts.hex[idx] });
                await waitChainData('freezeBp', accounts.hex[idx], 0);
                let accountAfter = await liteWeb.xlt.getUnconfirmedAccount(accounts.hex[idx]);
                ...
                await liteWeb.xlt.freezeBalance(10e5, 3, 'ENERGY', { privateKey: accounts.pks[idx], address: accounts.hex[idx] });
                await waitChainData('freezeEnergy', accounts.hex[idx], 0);
                accountAfter = await liteWeb.xlt.getUnconfirmedAccount(accounts.hex[idx]);
                assert.equal(
            ...
            it('should throw invalid resource provided: expected "BANDWIDTH" or "ENERGY" error', async function () {
                await assertThrow(
                    liteWeb.xlt.freezeBalance(10e8, 3, 'GAS', { privateKey: accounts.pks[idx], address: accounts.hex[idx] }),
            ...
            it('should throw invalid amount provided error', async function () {
                await assertThrow(
                    liteWeb.xlt.freezeBalance(-10, 3, 'BANDWIDTH', { privateKey: accounts.pks[idx], address: accounts.hex[idx] }),
                    'Invalid amount provided'
                );
            });
            ...
            it('should throw invalid duration provided, minimum of 3 days error', async function () {
                await assertThrow(
                    liteWeb.xlt.freezeBalance(10e8, 2, 'BANDWIDTH', { privateKey: accounts.pks[idx], address: accounts.hex[idx] }),
        ...
        describe.skip("#unfreezeBalance", async function () {
            ...
            before(async function(){
                await liteWeb.xlt.freezeBalance(10e5, 3, 'BANDWIDTH', {}, accounts.b58[15]);
                await liteWeb.xlt.freezeBalance(10e5, 3, 'ENERGY', {}, accounts.b58[15]);
            });

            it('should unfreeze balance', async function () {
                let accountBefore = await liteWeb.xlt.getUnconfirmedAccount(ADDRESS_HEX);
                await liteWeb.xlt.unfreezeBalance('BANDWIDTH', {}, accounts.b58[15]);
                let accountAfter = await liteWeb.xlt.getUnconfirmedAccount(ADDRESS_HEX);
                assert.equal(accountBefore.frozen[0].frozen_balance - 10e5, accountAfter.frozen[0].frozen_balance);

                accountBefore = accountAfter;
                await liteWeb.xlt.unfreezeBalance('ENERGY', {}, accounts.b58[15]);
                accountAfter = await liteWeb.xlt.getUnconfirmedAccount(ADDRESS_HEX);
                assert.equal(
                    accountBefore.account_resource.frozen_balance_for_energy.frozen_balance - 10e5,
                    accountAfter.account_resource.frozen_balance_for_energy.frozen_balance
                );
            ...
            it('should throw invalid resource provided: expected "BANDWIDTH" or "ENERGY" error', async function () {
                await assertThrow(
                    liteWeb.xlt.unfreezeBalance(10e8, 3, 'GAS', {}, accounts.b58[15]),
                    'Invalid resource provided: Expected "BANDWIDTH" or "ENERGY"'
                );
            });
        ...
        */
        ...
        describe("#broadcast", async function () {
            ...
            before(async function () {
                transaction = await liteWeb.transactionBuilder.freezeBalance(10e5, 3, 'BANDWIDTH', accounts.b58[idx]);
                signedTransaction = await liteWeb.xlt.sign(transaction, accounts.pks[idx]);
            });
            ...
            it('should throw invalid options provided error', async function () {
                await assertThrow(
                    liteWeb.xlt.broadcast(signedTransaction, false),
            ...
            it('should throw transaction is not signed error', async function () {
                await assertThrow(
                    liteWeb.xlt.broadcast(transaction),
        ...
        describe("#getTransaction", async function () {
            ...
            before(async function(){
                ...
                transaction = await liteWeb.xlt.freezeBalance(10e5, 3, 'BANDWIDTH', { privateKey: accounts.pks[idx], address: accounts.hex[idx] });
                transaction = transaction.transaction;
                await waitChainData('tx', transaction.txID);
            });

            it('should get transaction by id', async function () {
                const tx = await liteWeb.xlt.getTransaction(transaction.txID);
                assert.equal(tx.txID, transaction.txID);
            });

            it('should throw transaction not found error', async function () {
                await assertThrow(
                    liteWeb.xlt.getTransaction('a8813981b1737d9caf7d51b200760a16c9cdbc826fa8de102386af898048cbe5'),
        ...
        describe("#getTransactionFromBlock", async function () {
            ...
            before(async function(){
                ...
                transaction = await liteWeb.xlt.freezeBalance(10e5, 3, 'BANDWIDTH', { privateKey: accounts.pks[idx], address: accounts.hex[idx] });
                transaction = transaction.transaction;
                const currBlock = await liteWeb.xlt.getBlock('latest');
            ...
            it('should get transaction from block', async function () {
                ...
                for (let i = currBlockNum; i < currBlockNum + 3;) {
                    try {
                        const tx = await liteWeb.xlt.getTransactionFromBlock(i, 0);
                        // assert.equal(tx.txID, transaction.txID);
            ...
            it('should throw transaction not found error by transaction from block', async function () {
                await assertThrow(
                    liteWeb.xlt.getTransactionFromBlock(currBlockNum - 1, 0),
            ...
            it('should throw block not found error by transaction from block', async function () {
                await assertThrow(
                    liteWeb.xlt.getTransactionFromBlock(currBlockNum + 50, 0),
                    'Block not found'
                );
            });
            ...
            it('should throw invalid index error by transaction from block', async function () {
                await assertThrow(
                    liteWeb.xlt.getTransactionFromBlock(currBlockNum, -1),
        ...
        describe("#getTransactionInfo (Confirmed)", async function () {
            ...
            it('should get transaction info by id', async function () {
                this.timeout(20000);
                while (true) {
                    const tx = await liteWeb.xlt.getTransactionInfo(transaction.txID);
        ...
        describe("#geUnconfirmedTransactionInfo", async function () {
            ...
            before(async function(){
                transaction = (await liteWeb.xlt.freezeBalance(10e5, 3, 'BANDWIDTH', { privateKey: accounts.pks[idx], address: accounts.hex[idx] })).transaction;
                await waitChainData('tx', transaction.txID);
            });
            ...
            it('should get unconfirmed transaction by id', async function () {
                this.timeout(10000)
                await wait(3)
                const tx = await liteWeb.xlt.getUnconfirmedTransactionInfo(transaction.txID);
                assert.equal(tx.id, transaction.txID);
            });

            it('should throw transaction not found error', async function () {
                await assertThrow(
                    liteWeb.xlt.getUnconfirmedTransactionInfo('a8813981b1737d9caf7d51b200760a16c9cdbc826fa8de102386af898048cbe5'),
                    'Transaction not found'
                );
            });
        ...
        describe("#getConfirmedTransaction", async function () {
            ...
            it('should get confirmed transaction by tx id', async function () {
                ...
                while (true) {
                    try {
                        const tx = await liteWeb.xlt.getConfirmedTransaction(transaction.transaction.txID);
    ...
    describe('#Token Test', function () {
        ...
        describe("#sendAsset", async function () {
            ...
            before(async function(){
                ...
                const transaction = await liteWeb.transactionBuilder.createToken(options, accounts.hex[fromIdx]);
                await broadcaster(null, accounts.pks[fromIdx], transaction);
                await waitChainData('token', accounts.hex[fromIdx]);
                token = await liteWeb.xlt.getTokensIssuedByAddress(accounts.hex[fromIdx]);
            });

            it('should send xlt by to address and verify account balance', async function () {
                this.timeout(20000);

                const assetBefore = (await liteWeb.xlt.getUnconfirmedAccount(accounts.hex[toIdx])).assetV2;
                await waitChainData('tokenById', token[Object.keys(token)[0]]['id']);
                await liteWeb.xlt.sendAsset(
                    accounts.hex[toIdx],
                    10e4,
                ...
                const assetAfter = (await liteWeb.xlt.getUnconfirmedAccount(accounts.hex[toIdx])).assetV2;
                assert.equal(!assetBefore ? 0 : assetBefore[0].value, assetAfter[0].value - 10e4);
            });

            it('should throw invalid recipient provided error', async function () {
                await assertThrow(
                    liteWeb.xlt.sendAsset(
                        'notValidAddress',
                        10e4,
                        token[Object.keys(token)[0]]['id'],
                        { privateKey: accounts.pks[fromIdx], address: accounts.hex[fromIdx] }
        ...
        describe("#getTokenFromID", async function () {
            ...
            before(async function(){
                ...
                const options = getTokenOptions();
                const transaction = await liteWeb.transactionBuilder.createToken(options, accounts.hex[idx]);
                await broadcaster(null, accounts.pks[idx], transaction);
                await waitChainData('token', accounts.hex[idx]);
            });

            it('should get token by name', async function () {
                const tokens = await liteWeb.xlt.listTokens(5, 0);
                for (let token of tokens) {
                    const tk = await liteWeb.xlt.getTokenFromID(token.id);
            ...
            it('should throw token does not exist error', async function () {
                await assertThrow(
                    liteWeb.xlt.getTokenFromID(1234565),
        ...
        describe("#getTokensIssuedByAddress", async function () {
            ...
            it('should get token by issued address', async function () {
                const tokens = await liteWeb.xlt.listTokens(5, 0);
                for (let token of tokens) {
                    const tk = await liteWeb.xlt.getTokensIssuedByAddress(token.owner_address);
            ...
            it('should throw invalid address provided error', async function () {
                await assertThrow(
                    liteWeb.xlt.getTokensIssuedByAddress('abcdefghijklmn'),
                    'Invalid address provided'
                );
            });
        ...
        describe("#listTokens", async function () {
            ...
            it('should list all tokens by limit', async function () {
                const tokens = await liteWeb.xlt.listTokens(10, 0);
        ...
        describe("#parseToken", async function () {
            ...
            it('should list all tokens by limit', async function () {
                ...
                for (let token of tokens) {
                    ...
                    token.name = liteWeb.fromUtf8(token.name);
                    token.abbr = liteWeb.fromUtf8(token.abbr);
                    token.description = token.description && liteWeb.fromUtf8(token.description);
                    token.url = liteWeb.fromUtf8(token.url);

                    const tk = liteWeb.xlt._parseToken(token);
    ...
    describe('#Exchange Test', function () {
        ...
        describe("#listExchanges", async function () {
            ...
            before(async function(){
                ...
                for (let i = idxS; i < idxE; i++) {
                    const options = getTokenOptions();
                    const transaction = await liteWeb.transactionBuilder.createToken(options, accounts.hex[i]);
                    await broadcaster(null, accounts.pks[i], transaction);
                    await waitChainData('token', accounts.hex[i]);
                    const token = await liteWeb.xlt.getTokensIssuedByAddress(accounts.hex[i]);
                    await waitChainData('tokenById', token[Object.keys(token)[0]]['id']);
                    await broadcaster(null, accounts.pks[i], await liteWeb.transactionBuilder.sendToken(
                        accounts.hex[toIdx],
                        10e4,
                        token[Object.keys(token)[0]]['id'],
                ...
                await broadcaster(
                    null,
                    accounts.pks[toIdx],
                    await liteWeb.transactionBuilder.createTokenExchange(tokenNames[0], 10e3, tokenNames[1], 10e3, accounts.hex[toIdx])
                );

            });

            it('should get exchange by id', async function () {
                const exchanges = await liteWeb.xlt.listExchanges();
        ...
        describe("#listExchangesPaginated", async function () {
            ...
            it('should get exchange by id', async function () {
                const exchanges = await liteWeb.xlt.listExchangesPaginated(10, 0);
                assert.isArray(exchanges);
        ...
        describe("#getExchangeByID", async function () {
            ...
            before(async function(){
                ...
                for (let i = idxS; i < idxE; i++) {
                    const options = getTokenOptions();
                    await broadcaster(null, accounts.pks[i], await liteWeb.transactionBuilder.createToken(options, accounts.hex[i]));
                    await waitChainData('token', accounts.hex[i]);
                    const token = await liteWeb.xlt.getTokensIssuedByAddress(accounts.hex[i]);
                    await waitChainData('tokenById', token[Object.keys(token)[0]]['id']);
                    await broadcaster(null, accounts.pks[i], await liteWeb.transactionBuilder.sendToken(
                ...
                exchanges = await liteWeb.xlt.listExchanges();
            });

            it('should get exchange by id', async function () {
                for (let exchange of exchanges) {
                    const ex = await liteWeb.xlt.getExchangeByID(exchange.exchange_id);
    ...
    describe("#Proposal Test", async function () {
        ...
        describe("#getChainParameters", async function () {
            ...
            it('should get proposal list', async function () {
                const params = await liteWeb.xlt.getChainParameters();
        ...
        describe("#getProposal", async function () {
            ...
            before(async function(){
                ...
                await broadcaster(
                    null,
                    PRIVATE_KEY,
                    await liteWeb.transactionBuilder.createProposal(parameters[0], ADDRESS_BASE58)
                );

                proposals = await liteWeb.xlt.listProposals();
            });

            it('should get proposal by id', async function () {
                for (let proposal of proposals) {
                    const ps = await liteWeb.xlt.getProposal(proposal.proposal_id);
        ...
        describe("#listProposals", async function () {
            ...
            it('should list seeds node', async function () {
                const proposals = await liteWeb.xlt.listProposals();
                for (let proposal of proposals) {
    ...
    describe("#getContract", async function () {
        ...
        before(async function(){
            ...
            transaction = await liteWeb.transactionBuilder.createSmartContract({
        ...
        it('should get contract by contract address', async function () {
            const contract = await liteWeb.xlt.getContract(transaction.contract_address);
        ...
        it('should throw invalid contract address provided error', async function () {
            await assertThrow(
                liteWeb.xlt.getContract('notAddress'),
        ...
        it('should throw contract does not exist error', async function () {
            await assertThrow(
                liteWeb.xlt.getContract('417cbcc41052b59584d1ac9fc1ce39106533aa1d40'),
    ...
    describe("#listNodes", async function () {
        ...
        it('should list seeds node', async function () {
            const nodes = await liteWeb.xlt.listNodes();
    ...
    describe("#listSuperRepresentatives", async function () {
        ...
        it('should list super representatives', async function () {
            const srs = await liteWeb.xlt.listSuperRepresentatives();
            assert.isArray(srs);
            for (let sr of srs) {
    ...
    describe("#timeUntilNextVoteCycle", async function () {
        ...
        it('should get time util next vote cycle', async function () {
            const num = await liteWeb.xlt.timeUntilNextVoteCycle();
            assert.isNumber(num);
        });
    ...
    describe("#getReward", async function () {
        it('should get the reward', async function () {
            ...
            let reward = await liteWeb.xlt.getReward(accounts[0]);
    ...
    describe("#getUnconfirmedReward", async function () {
        it('should get the reward', async function () {
            ...
            let reward = await liteWeb.xlt.getUnconfirmedReward(accounts[0]);
            assert.equal(reward, 0)
    ...
    describe("#getBrokerage", async function () {
        it('should get the brokerage', async function () {
            ...
            let brokerage = await liteWeb.xlt.getBrokerage(accounts[0]);
    ...
    describe("#getUnconfirmedBrokerage", async function () {
        it('should get the brokerage', async function () {
            ...
            let brokerage = await liteWeb.xlt.getUnconfirmedBrokerage(accounts[0]);
            assert.equal(brokerage, 20)
