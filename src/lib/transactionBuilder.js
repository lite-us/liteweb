...
function toHex(value) {
    return LiteWeb.address.toHex(value);
}

function fromUtf8(value) {
    return self.liteWeb.fromUtf8(value);
}
...
function resultManager(transaction, callback) {
    ...
    if (transaction.result && transaction.result.message) {
        return callback(
            self.liteWeb.toUtf8(transaction.result.message)
        );
    }
...
export default class TransactionBuilder {
    constructor(liteWeb = false) {
        ...
        this.validator = new Validator(liteWeb);
    }

    sendXlt(to = false, amount = 0, from = this.liteWeb.defaultAddress.hex, options, callback = false) {
        ...
        if (!callback)
            return this.injectPromise(this.sendXlt, to, amount, from, options);
        ...
        if (this.validator.notValid([
            ...
            },
            {
                ...
                type: 'notEqual',
                msg: 'Cannot transfer XLT to the same account'
            },
            {
                name: 'amount',
                type: 'integer',
        ...
        this.liteWeb.fullNode.request('wallet/createtransaction', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }

    sendToken(to = false, amount = 0, tokenID = false, from = this.liteWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        this.liteWeb.fullNode.request('wallet/transferasset', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }

    purchaseToken(issuerAddress = false, tokenID = false, amount = 0, buyer = this.liteWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        if (utils.isFunction(buyer)) {
            callback = buyer;
            buyer = this.liteWeb.defaultAddress.hex;
        ...
        this.liteWeb.fullNode.request('wallet/participateassetissue', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }

    freezeBalance(amount = 0, duration = 3, resource = "BANDWIDTH", address = this.liteWeb.defaultAddress.hex, receiverAddress = undefined, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        if (utils.isFunction(address)) {
            callback = address;
            address = this.liteWeb.defaultAddress.hex;
        ...
        this.liteWeb.fullNode.request('wallet/freezebalance', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }

    unfreezeBalance(resource = "BANDWIDTH", address = this.liteWeb.defaultAddress.hex, receiverAddress = undefined, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        this.liteWeb.fullNode.request('wallet/unfreezebalance', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }

    withdrawBlockRewards(address = this.liteWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        this.liteWeb.fullNode.request('wallet/withdrawbalance', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }
    ...
    applyForSR(
        ...
        this.liteWeb.fullNode.request('wallet/createwitness', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }

    vote(votes = {}, voterAddress = this.liteWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        if (utils.isFunction(voterAddress)) {
            ...
            voterAddress = this.liteWeb.defaultAddress.hex;
        ...
        this.liteWeb.fullNode.request('wallet/votewitnessaccount', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }

    createSmartContract(options = {}, issuerAddress = this.liteWeb.defaultAddress.hex, callback = false) {
        if (utils.isFunction(issuerAddress)) {
            callback = issuerAddress;
            issuerAddress = this.liteWeb.defaultAddress.hex;
        }
        ...
        // tokenValue and tokenId can cause errors if provided when the xlt10 proposal has not been approved yet. So we set them only if they are passed to the method.
        ...
        this.liteWeb.fullNode.request('wallet/deploycontract', args, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }
    ...
    _triggerSmartContract(
        ...
        this.liteWeb[options.confirmed ? 'solidityNode' : 'fullNode'].request(`wallet${options.confirmed ? 'solidity' : ''}/trigger${options._isConstant ? 'constant' : 'smart'}contract`, args, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }

    clearABI(contractAddress, ownerAddress = this.liteWeb.defaultAddress.hex, callback = false) {
        if (!callback)
            return this.injectPromise(this.clearABI, contractAddress, ownerAddress);

        if (!this.liteWeb.isAddress(contractAddress))
            return callback('Invalid contract address provided');

        if (!this.liteWeb.isAddress(ownerAddress))
        ...
        if (this.liteWeb.xlt.cache.contracts[contractAddress]) {
            delete this.liteWeb.xlt.cache.contracts[contractAddress]
        }
        this.liteWeb.fullNode.request('wallet/clearabi', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));

    }

    updateBrokerage(brokerage, ownerAddress = this.liteWeb.defaultAddress.hex, callback = false) {
        if (!callback)
        ...
        this.liteWeb.fullNode.request('wallet/updateBrokerage', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));

    }

    createToken(options = {}, issuerAddress = this.liteWeb.defaultAddress.hex, callback = false) {
        if (utils.isFunction(issuerAddress)) {
            callback = issuerAddress;
            issuerAddress = this.liteWeb.defaultAddress.hex;
        }
        ...
        if (this.liteWeb.fullnodeSatisfies('>=3.5.0') && !(parseInt(frozenAmount) > 0)) {
        ...
        this.liteWeb.fullNode.request('wallet/createassetissue', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }

    updateAccount(accountName = false, address = this.liteWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        this.liteWeb.fullNode.request('wallet/updateaccount', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }

    setAccountId(accountId, address = this.liteWeb.defaultAddress.hex, callback = false) {
        if (utils.isFunction(address)) {
            callback = address;
            address = this.liteWeb.defaultAddress.hex;
        }
        ...
        this.liteWeb.fullNode.request('wallet/setaccountid', {
            account_id: accountId,
            owner_address: toHex(address),
        }, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }

    updateToken(options = {}, issuerAddress = this.liteWeb.defaultAddress.hex, callback = false) {
        if (utils.isFunction(issuerAddress)) {
            callback = issuerAddress;
            issuerAddress = this.liteWeb.defaultAddress.hex;
        ...
        this.liteWeb.fullNode.request('wallet/updateasset', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }
     ...
     */
    createProposal(parameters = false, issuerAddress = this.liteWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        this.liteWeb.fullNode.request('wallet/proposalcreate', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }
     ...
     */
    deleteProposal(proposalID = false, issuerAddress = this.liteWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        this.liteWeb.fullNode.request('wallet/proposaldelete', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }
     ...
     * Only current Super Representative can vote on a proposal.
     */
    voteProposal(proposalID = false, isApproval = false, voterAddress = this.liteWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        this.liteWeb.fullNode.request('wallet/proposalapprove', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }

    /**
     * Create an exchange between a token and XLT.
     * Token Name should be a CASE SENSITIVE string.
     * PLEASE VERIFY THIS ON LITESCAN.
     */
    createXLTExchange(tokenName, tokenBalance, xltBalance, ownerAddress = this.liteWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        if (utils.isFunction(ownerAddress)) {
            ...
            ownerAddress = this.liteWeb.defaultAddress.hex;
        ...
        if (!callback)
            return this.injectPromise(this.createXLTExchange, tokenName, tokenBalance, xltBalance, ownerAddress, options);
        ...
        this.liteWeb.fullNode.request('wallet/exchangecreate', data, 'post').then(resources => {
     ...
     * PLEASE VERIFY THIS ON LITESCAN.
     */
    createTokenExchange(firstTokenName, firstTokenBalance, secondTokenName, secondTokenBalance, ownerAddress = this.liteWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
     ...
     */
    injectExchangeTokens(exchangeID = false, tokenName = false, tokenAmount = 0, ownerAddress = this.liteWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        this.liteWeb.fullNode.request('wallet/exchangeinject', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }
     ...
     * Use "_" for the constant value for XLT.
     */
    withdrawExchangeTokens(exchangeID = false, tokenName = false, tokenAmount = 0, ownerAddress = this.liteWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        this.liteWeb.fullNode.request('wallet/exchangewithdraw', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }
    ...
    tradeExchangeTokens(exchangeID = false,
        ...
        const data = {
            owner_address: toHex(ownerAddress),
            exchange_id: parseInt(exchangeID),
            token_id: this.liteWeb.fromAscii(tokenName),
        ...
        this.liteWeb.fullNode.request('wallet/exchangetransaction', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }
    ...
    updateSetting(contractAddress = false,
        ...
        this.liteWeb.fullNode.request('wallet/updatesetting', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }
    ...
    updateEnergyLimit(contractAddress = false,
        ...
        this.liteWeb.fullNode.request('wallet/updateenergylimit', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }
    ...
    checkPermissions(permissions, type) {
        if (permissions) {
            ...
            for (let key of permissions.keys) {
                if (!this.liteWeb.isAddress(key.address)
    ...
    updateAccountPermissions(ownerAddress = this.liteWeb.defaultAddress.hex,
        ...
        this.liteWeb.fullNode.request('wallet/accountpermissionupdate', data, 'post').then(transaction => resultManager(transaction, callback)).catch(err => callback(err));
    }
    ...
    async alterTransaction(transaction, options = {}, callback = false) {
        ...
        if (options.data) {
            if (options.dataFormat !== 'hex')
                options.data = this.liteWeb.toHex(options.data);
